...
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n > 3.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >4.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >5.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >6.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >7.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >8.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >9.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >12.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >13.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >14.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >15.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >16.txt
cat files.txt  | grep -o "\.[A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z][A-Za-z]$"  | sort |uniq -c | sort -n >18.txt
...
| latex    |      1 |
#+NAME: table-sum
#+BEGIN_SRC python :var table=source-table :export both
...
To traverse the current Org document in Emacs using Org Babel and find the previous block of data for parsing, you can create an inline Emacs Lisp code block within your Org document. Here's an example of how to do this:


#+BEGIN_SRC emacs-lisp :var current-block="" :exports none
  (let ((parsed-data ""))
    (org-element-map (org-element-parse-buffer) 'src-block
      (lambda (src-block)
        (message (org-babel-lob-call src-block))
        (when (string= "data" (org-element-property :name src-block))
          (setq parsed-data (org-babel-lob-call src-block)))
        ))
    parsed-data)
#+END_SRC
...
1. We use an Emacs Lisp code block (`#+BEGIN_SRC emacs-lisp`) to define our inline script.

2. Within the code block, we use `org-element-map` to traverse the parsed Org document.

3. We look for `src-block` elements and check if they have the `:name` property set to "data." This indicates a block of data in the Org document.

4. If we find a matching block, we use `org-babel-lob-call` to execute the code in that block and store the result in the `parsed-data` variable.

5. Finally, we return the `parsed-data`.

You can then use this inline script by inserting it into your Org document where you need to extract and parse data from the previous data block. You can customize the `:name` property and the code within your "data" block according to your specific needs.
...
scores = Counter(row[1] for row in table)
...
4. for variations ignoring whitespace or other changes replace one line with another to fill in missing.
